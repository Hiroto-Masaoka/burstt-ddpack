...
# Functions
# ============================================================
def load_bin_files(idir):
    if not os.path.isdir(idir):
        print(f"[WARN] Directory not found: {idir}")
        return []

    # Get *.bin
    files = glob(os.path.join(idir, "*.bin"))
    if not files:
        print(f"[INFO] No .bin files found in {idir}")
        return []
    
    # Sort files based on o<number>.bin
    def extract_order(f):
        m = re.search(r'o(\d+)\.bin$', os.path.basename(f))
        return int(m.group(1)) if m else -1

    files = sorted(files, key=extract_order)
    nFile = len(files)

    print(f"[INFO] Found {nFile} .bin files in {idir}")
    for f in files:
...
parser = argparse.ArgumentParser(
    prog="auto_beamform_FUSHAN_fix.py",
    usage="%(prog)s csvfile [--tstart TSTART --tend TEND --station STATION]",
    description="Read triggered baseband data and execute 2nd beamforming with a Chih-Yi's Fast Assenbly Code & Kyle's 2nd-bf optimization",
    epilog="Example:\n  python %(prog)s triggers.csv --tstart '2025-10-01 00:00:00' --tend '2025-10-01 01:00:00' --station Fushan --dcal",
...
for idx, row in df.iterrows():
    ...
    # Pick up the latest calibration file
...
############################
        ...
        with open(fbin, 'rb') as fh:
            ...
            #p1 = int((ep_h - secWin - ep_begin) / (frame_per_pack * timeFrame)) * nRow
            #p2 = int((ep_l + secWin - ep_begin) / (frame_per_pack * timeFrame)) * nRow
            nPack = p2 - p1
    ...
    gs = gridspec.GridSpec(2, 3, width_ratios=[20, 1, 0.5], height_ratios=[4, 1], hspace=0.05, wspace=0.05)
    ...
    # Finalize
    plt.tight_layout()
    plt.savefig(f'pulse_dedispersion_combined_{ev_name}.png', dpi=300)
    #plt.show()
